...
function EmployeeListPage() {
  ...
  const { items, pagination, listStatus, deleteStatus, error } = useSelector(
  ...
  async function handleDelete(employee) {
    const confirmed = window.confirm(
      `Delete ${employee.fullName} (${employee.employeeId}) and all related attendance, leave, and payroll records?`,
    )
    ...
    const action = await dispatch(deleteEmployee(employee.id))
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="rounded-full border border-rose-400/25 px-3 py-2 text-xs font-semibold text-rose-100 transition hover:bg-rose-400/10 disabled:opacity-50"
                      >
                        Delete
                      </button>
                    </div>
